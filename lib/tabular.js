(function () {
  ...
  function columnize(data, headers, dot) {
    function recurse(val, path, hdr) {
      if (Object(val) !== val || val instanceof Date) {
        insert(val, path, hdr);
  ...
  var tabulate = {
    delimit: function(data, options) {
      ...
      var escapeString = typeof options.escape === 'function' ? options.escape : function(string, wrap) {
      ...
      var string = rows.map(function(n) {
        return n.map(function(v) {
          if (typeof v === 'string') {return stringWrap+escapeString(v, stringWrap).replace(/[\n\r]+/g, ' ')+stringWrap;}
    ...
    html: function(data, options) {
      ...
      options = typeof options !== 'undefined' ? options : {};
      options.includeHeaders = true;
      options.dot = typeof options.dot !== 'undefined' ? options.dot : ' ';
      var dateFormatter = typeof options.dateFormatter !== 'undefined' ? options.dateFormatter : function(date) {return date.toLocaleDateString();};
      var classes = typeof options.classes !== 'undefined' ? options.classes : {};
      var eol = typeof options.eol !== 'undefined' ? options.eol : '\n';
      var separator = typeof options.separator !== 'undefined' ? options.separator : '\t';
      var array = this.array(data, options);
      var allowHTML = typeof options.allowHTML !== 'undefined' ? options.allowHTML : false;

      if (array.length < 2) {return '';}

      var result = '';
      result += '<table'+(typeof classes.table !== 'undefined' ? ' class="'+classes.table+'"' : '')+'>' + eol;
      result += separator+'<thead'+(typeof classes.thead !== 'undefined' ? ' class="'+classes.thead+'"' : '')+'>' + eol;
      result += separator+separator+'<tr'+(typeof classes.tr !== 'undefined' ? ' class="'+classes.tr+'"' : '')+'>' + eol;
      for (var i=0; i<array[0].length; i++) {
        result += separator+separator+separator+'<th'+(typeof classes.th !== 'undefined' ? ' class="'+classes.th+'"' : '')+'>';
        result += allowHTML ? array[0][i] : htmlEntities(array[0][i]);
        result += '</th>' + eol;
      }
      result += separator+separator+'</tr>' + eol;
      result += separator+'</thead>' + eol;
      result += separator+'<tbody'+(typeof classes.tbody !== 'undefined' ? ' class="'+classes.tbody+'"' : '')+'>' + eol;
      for (var row=1; row<array.length; row++) {
        result += separator+separator+'<tr'+(typeof classes.tr !== 'undefined' ? ' class="'+classes.tr+'"' : '')+'>' + eol;
        for (var col=0; col<array[row].length; col++) {
          result += separator+separator+separator+'<td'+(typeof classes.td !== 'undefined' ? ' class="'+classes.td+'"' : '')+'>';
          if (array[row][col] instanceof Date) {
            result += dateFormatter(array[row][col]);
          }  else {
            result += allowHTML ? array[row][col] : htmlEntities(array[row][col]);
          }
          result += '</td>' + eol;
        }
        result += separator+separator+'</tr>' + eol;
